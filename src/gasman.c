...
#endif
  AllocBags += WORDS_BAG(size) + HEADER_SIZE;
  /* The 3 is for the size, typeflags and link words */
#ifdef DEBUG_LOADING
...
again:
    ...
    /* if we already performed a full garbage collection                   */
      ...
      for (p = MptrBags; p < OldBags; p+= SIZE_MPTR_BAGS)
        ...
        while ( ( EndBags < StopBags + (StopBags-OldBags)/7 ||
		  EndBags - StopBags < AllocSizeBags )
             && (*AllocFuncBags)(512*1024L,0) )
            EndBags += WORDS_BAG(512*1024L);

	/* If we are having trouble, then cur our cap to fit our cloth *.
	...
	AllocSizeBags = 7*(Endbags - StopBags)/8; */
		...
		&& EndBags - StopBags > AllocSizeBags + WORDS_BAG(512*1024L)
             && (*AllocFuncBags)(-512*1024L,0) )
            EndBags -= WORDS_BAG(512*1024L);
