...
**      8. TypePlistHom     -- also handles Tab
**      9. TypePlistCyc
**      10.TypePlistFfe
**
**     Of these:
...
**
**     recursive lists (ie lists which are there own subjects are detected
    ...
    Obj                 *famfirst)
{
    ...
    for ( i = 2; isDense && (isHom || ! areMut) && i <= lenList; i++ ) {
        ...
        if ( elm == 0 ) {
            ...
            isTable = (isTable && IS_SMALL_LIST(elm) && LEN_LIST(elm) == len);
        }
    }
    ...
    Obj                 list)
{
    ...
    if (!HAS_FILT_LIST(list, FN_IS_TABLE ))
      {
	if ( IS_HOMOG_LIST(elm) && IS_SMALL_LIST(elm) ) {
	  isTable = 1;
	  len     = LEN_LIST(elm);
	}
