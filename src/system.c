...
# if !GENERATINGPOWERPC /* on the Power Mac, the stack size is set in the project */
static UInt syStackSpace = 64 * 1024;
# endif
#endif
...
extern long         syIsIntrTime;
extern long         syIsIntrFreq; 
extern Boolean		syCanExec, syCanLoadDynamicModules;
extern Int 			SyInFid, SyOutFid; /* for i/o redirection */
UInt				SyGaprc = 1;
FSSpec 				syOptionsFSSpec; /* file specs for options file */
...
# endif
	...
	if (err)
		err = FSMakeFSSpec (0, 0, "\pgap.options", &syOptionsFSSpec);
	else
		err = FSMakeFSSpec (s, k, "\pGAP options", &syOptionsFSSpec);
	if (err == noErr || err == fnfErr)
		err = FSSpecToPath (&syOptionsFSSpec, &syOptionsPath, sizeof (syOptionsPath), 
			true, err == fnfErr);
	if (err != noErr && err != fnfErr)
		syOptionsPath[0] = '\0';
...
# endif
	...
	max = (Int)((MaxMem( &mem ) - gEditorScratch - gMaxLogSize - pre));  

	if (SyStorMax > max)
		SyStorMax = max;
		
	/* make SyStorMax divisible by the minimum allocatable unit */
#if GAPVER == 4
	SyStorMax -= SyStorMax % (512L * 1024L);
#elif GAPVER == 3
	SyStorMax -= SyStorMax % 1024;
#endif
	...
	syWorkspace = (UInt***) NewPtr (SyStorMax);  /* allocate all we can get */
    ...
    if ( SyStorMax < SyStorMin || !syWorkspace) {
        ...
        "     to increase the minimum amount of memory and the preferred amount of memory\n", 3);
            SyFputs (
        "     as described in the documentation of GAP for the Mac.\n", 3 );
            SyExit( 1 );
    }
	if (SyBanner && !SyCompilePlease) {
		OpenAboutBox (5);   /* show GAP's About ... box for 5 seconds*/
		SyFputs ("Loading the library. Please be patient, this may take a while.\n", 1);
	}
