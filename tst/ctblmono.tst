...
gap> chi:= Irr( Sl23 )[7];
Character( SL(2,3), [ 3, 0, 0, 3, 0, 0, -1 ] )
...
1
gap> chi:= Irr( Sl23 )[4];
Character( SL(2,3), [ 2, 1, 1, -2, -1, -1, 0 ] )
gap> cln:= ClassesOfNormalSubgroup( CharacterTable( Sl23 ), n );
[ 1, 4, 7 ]
...
[ rec( isInduced := false, comment := "linear character" ), 
  rec( isInduced := true, comment := "induced from component '.character'", 
      character := Character( Group([ (), (1,2)(3,4), (1,2,3) ]), 
        [ 1, E(3)^2, E(3), 1 ] ) ), 
...
rec( isSubnormallyMonomial := false, 
  character := Character( S4, [ 3, -1, -1, 0, 1 ] ), 
...
gap> STOP_TEST( "ctblmono.tst", 279346138 );
