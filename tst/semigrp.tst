...
gap> C := MagmaCongruenceByGeneratingPairs(O4, [[a*f, a*e]]);;
gap> P := EquivalenceRelationPartition(C);;
gap> I = P[1];                         # true
true
gap> Elements(I) = Elements(P[1]);     # true
true
...
gap> Size(M4);
35
gap> congM4 := MagmaCongruenceByGeneratingPairs(M4, [[eM4[6],eM4[9]]]);;
gap> QM4 := M4/congM4;;
gap> One(a);;
gap> One(QM4);;
gap> One(eM4[2]);;
gap> eqm4 := Elements(QM4);;
gap> Size(QM4);
2
gap> One(eqm4[2]);;
gap> 
...
gap> S := Semigroup(s,t);;
gap> e := Elements(S);;
gap> c := MagmaCongruenceByGeneratingPairs(S,[[e[9],e[12]]]);;
