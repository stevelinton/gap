...
#############################################################################
##
#M  ProjectiveGeneralLinearGroupCons( <IsPermGroup>, <n>, <q> )
##
InstallMethod( ProjectiveGeneralLinearGroupCons,"action on lines",true,
    [ IsPermGroup, IsPosInt,IsPosInt ], 0,
function(fil,n,q)
local pg,g,o;
  g:=GeneralLinearGroup(IsMatrixGroup,n,q);
  o:=ListWithIdenticalEntries(n,0*Z(q));
  o[1]:=Z(q)^0;
  ConvertToVectorRep(o);
  o:=Immutable(o);
  o:=Orbit(g,o,OnLines);
  o:=Set(o);
  pg:=Action(g,o,OnLines);
  SetSize(pg,Size(g)/(q-1));
  return pg;
end);

#############################################################################
##
#M  ProjectiveSpecialLinearGroupCons( <IsPermGroup>, <q>, <n> )
##
InstallMethod( ProjectiveSpecialLinearGroupCons,"action on lines",true,
    [ IsPermGroup, IsPosInt,IsPosInt ], 0,
function(fil,n,q)
local pg,g,o;
  g:=SpecialLinearGroup(IsMatrixGroup,n,q);
  o:=ListWithIdenticalEntries(n,0*Z(q));
  o[1]:=Z(q)^0;
  ConvertToVectorRep(o);
  o:=Immutable(o);
