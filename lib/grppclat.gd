...
##  as follows: The arguments are subgroups of $G$ in the relation
##  $C>A>N>B>M$. $N$ and $M$ are normal subgroups. <C> is the full preimage
##  of the normalizer of <A>/<N> in <G>/<N>. When computing modulo <M> and
##  looking for subgroups <U> such that $U\cap N=B$ and $\langle
##  U,N\rangle=A$, this function is called. If it returns `false' all
##  potential groups <U> (and therefore all groups later arising from them)
##  are disregarded. This can be used for example to compute only subgroups
##  of certain sizes. 
##  {}
...
##  {}
##  The actual groups <C>, <A>, <N> and <B> which are passed to
##  this function are not necessarily subgroups of <G> but might be
##  subgroups of a proper factor group <G>/<H>. Therefore the `consider'
