...
InstallMethod( OrbitOp, "with domain (ignored)", true, OrbitishReq,0,
function( G, D, pnt, gens, acts, act )
    return OrbitOp( G, pnt, gens, acts, act );
end );
...
local   orbstab;
  orbstab :=
  OrbitStabilizerAlgorithm(G,false,false,gens,acts,rec(pnt:=pnt,act:=act));
  return Immutable( orbstab );
end );
...
local   orb,  stb,  rep,  p,  q,  img,  sch,  i,d,act,
  ...
  if D=false then
    ...
    if blist<>false then
      ...
      blico:=ShallowCopy(blist); # the original indices
      blif:=true;
    else
      doml:=Size(D);
    fi;

    # Do we want to use a blist instead of testing in the orbit?
    useblist:=IsQuickPositionList(D);

    # do we have to crate a blist?
    if useblist and not blif then
  ...
  if blif or useblist then
    q := PositionCanonical( D, d ); 
    blist[q]:=true; # tick off first
    crossind[q]:=1;
  fi;
  ...
  if not IsEmpty( acts )  then
	  ...
	  for i  in [ 1 .. Length( gens ) ]  do
	    ...
	    if useblist then
	      # use blists if position test is quicker in D
	      q := PositionCanonical( D, img ); 
	      if blist[q] then
		q:=crossind[q]; # translate to `orb' position
	      else
		blist[q]:=true;
		crossind[q]:=Length(orb)+1; # the `orb' position
		q:=fail;
	      fi;
	    else
	      q := Position( orb, img );
		...
		if indh<Length(orb) then
	      ...
	      if blif and not useblist then
	        # a blist was given but we do not use it for the orbit. We
		# have to tick off elements at least once
		...
		if not blif then
		  ...
		  blif:=false; # hack to avoid the `PositionCanonical
		  # test below
		fi;
	      fi;

	      if  ind=Length(orb) then
...
InstallMethod( SparseActionHomomorphismOp,
    ...
    od;
    imgs := List( list, PermList );
    hom := ActionHomomorphism( G, D{start}, gens, acts, act,"surjective" );
    SetAsGroupGeneralMappingByImages( hom, GroupHomomorphismByImagesNC
            ( G, Range(hom), gens, imgs ) );
    return hom;
end );
...
function( G, start, gens, acts, act )
    ...
    od;
    imgs := List( list, PermList );
    hom := ActionHomomorphism( G, start, gens, acts, act,"surjective" );
    SetAsGroupGeneralMappingByImages( hom, GroupHomomorphismByImagesNC
            ( G, Range(hom), gens, imgs ) );
    return hom;
end );
...
#############################################################################
##
#F  SortedSparseActionHomomorphism( <arg> )
##
...
local ptset,p,i,img,imgs,hom;
    ...
    # the cost of doing the action a second time. So we create it anew.
    # (note that ptset is sorted, so `Position' used (for
    # `PositionCanonical') in `Permutation' will actually call
    # `POSITION_SORTED_LIST'.

    imgs:=List(acts,i->Permutation(i,ptset,act));

    hom := ActionHomomorphism( G, ptset, gens, acts, act,"surjective" );
    SetAsGroupGeneralMappingByImages( hom, GroupHomomorphismByImagesNC
            ( G, Range(hom), gens, imgs ) );
    return hom;
end );
...
local   stb,  p,  orbstab;
  ...
  if     IsIdenticalObj( gens, acts )
    ...
    # for tuples compute the stabilizer iteratedly
    stb := G;
    for p  in d  do
...
##
InstallMethod( PreImagesRepresentative,"IsLinearActionHomomorphism", true,
        [ IsLinearActionHomomorphism, IsPerm ], 0,
    function( hom, elm )
    local   V,  base,  mat,  b;
