...
InstallMethod( IsBergerCondition,
    ...
    if Size( G ) mod 2 = 1 then
      ...
      for psi in List( Irr( G ), ValuesOfClassFunction ) do
        ...
        if pos = fail then
          Add( degrees, psi[1] );
          Add( kernels, ShallowCopy( KernelChar( psi ) ) );
        else
...
InstallGlobalFunction( TestHomogeneous, function( chi, N )
    ...
    if IsList( N ) then
      ...
      cl:= ClassesOfNormalSubgroup( UnderlyingCharacterTable( chi ), N );
    fi;
...
#############################################################################
##
#M  IsPrimitiveProp( <chi> )  . . . . . . . . . .  for a character with group
##
InstallOtherMethod( IsPrimitiveProp,
    "for a character with group",
    true,
    [ IsCharacter and IsClassFunctionWithGroup ], 0,
...
#T the product of `N' and kernel of `chi'?
          ...
          n:= ClassesOfNormalSubgroup( OrdinaryCharacterTable( G ), N );
          ...
          if norm = Size( N ) then
            ...
            test:= rec( isRelativelySM := true,
                        ...
                        character      := CharacterByValues( G, chi ) );
...
InstallOtherMethod( TestRelativelySM,
    "for a character with group, and an object",
    true,
    [ IsCharacter and IsClassFunctionWithGroup, IsObject ], 0,
    TestRelativelySMFun );
