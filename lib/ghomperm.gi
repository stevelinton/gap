...
##
InstallOtherMethod( IsConjugatorAutomorphism, "perm group homomorphism",true,
  [IsGroupGeneralMapping],
  # there is no filter to test whether a homomorphsim goes from a perm group
  # to a perm group. So we have to test explicitly and make this method
  # higher ranking than the default one in `ghom.gi'.
  1,
function(hom)
local s, rep,dom,insn,stb,E,bpt,fix,pnt,idom,sliced,
      o,oimgs,i,pi,sto,stbs,stbi;
  s:=Source(hom);
  ...
  if rep<>fail then
	...
	od;
      ...
      # we got 
      rep:=RepresentativeOperation(OrbitStabilizingParentGroup(s),
            GeneratorsOfGroup(s),
	    List(GeneratorsOfGroup(s),i->ImagesRepresentative(hom,i)),OnTuples);
    fi;
  fi;
  ...
  if rep<>fail then
    Assert(1,ForAll(GeneratorsOfGroup(s),i->Image(hom,i)=i^rep));
