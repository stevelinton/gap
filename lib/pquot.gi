...
#############################################################################
##
#M  InitialiseCentralRelations . . . . . . . . . initialise central relations
##
...
function( qs )
    ...
    for i in qs!.nonDefiningImages do
        ...
        pair[2] := ExtRepByTailVector( qs, ReducedVectorLTM( M, pair[2] * one ) );
        qs!.images[i] := 
...
#        od;
        while CollectWordOrFail( S, v, rr ) = fail do
            Info( InfoQuotientSystem, 3, 
                  ...
                  " and was restarted" );
        od;
        AddVectorLTM( LTM, v{[n+1..n+c]} * one );
...
function( qs )
    ...
    for r in RelatorsOfFpGroup( G ) do
        AddVectorLTM( LTM, ExponentSums( r ) * one );
    od;
    ...
    l := 0;
    for i in [1..Length(LTM.matrix)] do
