...
Matrix_CharacteristicPolynomialSameField := function( fld, mat, ind )
    ...
    P   := [ ind^0 ];
    ind := IndeterminateNumberOfLaurentPolynomial(ind);
...
function ( mat )
    ...
    rank:= RankMat( mat );
    if rank = fail then
      if IsUnit( DeterminantMat( mat ) ) then
...
##
InstallMethod( DeterminantMat, "fraction-free method", true,
        [ IsOrdinaryMatrix  ],0,
        function ( mat )
    local   det, sgn, row, zero, m, i, j, k, l,mult, row2, piv;
...
InstallOtherMethod( ElementaryDivisorsMat,"compatibility method for integers",
    true, [ IsMatrix ], 0, mat->ElementaryDivisorsMat(Integers,mat));

#############################################################################
##
#F  MutableCopyMat( <mat> ) . . . . . . . . . .  transposed of a matrix
##
InstallGlobalFunction( MutableCopyMat, function( mat )
...
InstallMethod( RankMat,
    ...
    0,
        function(mat)
    local n,i,z,j,t,rk,l,k,piv,row,x,m;
...
InstallGlobalFunction( DirectSumMat, function (arg)                     
    ...
    res:=List(NullMat(Sum(arg,Length),Sum(arg,f)),ShallowCopy);
    r:=0;
    c:=0;          
    for m in arg do   
