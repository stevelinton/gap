...
##
InstallMethod( CorrespondingPermutations,
    "for a class function and a list of group elements",
    true,
    [ IsOrdinaryTable, IsClassFunction, IsHomogeneousList ], 0,
    function( tbl, chi, elms )
...
InstallMethod( NaturalCharacter,
    ...
    if IsPermGroup( R ) then
      ...
      return Character( tbl,
                 List( ConjugacyClasses( tbl ),
                 C -> deg - NrMovedPointsPerm( ImagesRepresentative( hom,
                                Representative( C ) ) ) ) );
...
#T !!
      ...
      if not IsBound( CONWAYPOLYNOMIALS[p][k] ) then
        Info( InfoCharacterTable, 1,
              "trying to cpmpute Conway polynomial of degree ", k,
              " for p = ", p );
      fi;
...
InstallOtherMethod( LeftModuleByGenerators,
    "for division ring and homog. list (of class functions)",
    true,
    [ IsDivisionRing, IsHomogeneousList ] , 30,
#T increase the rank, for preferring to Gaussian space construction
    function( F, gens )
    local tbl, V;
    ...
    V!.elementsunderlying:= tbl;
    return V;
    end );

InstallOtherMethod( LeftModuleByGenerators,
    "for division ring, homog. list (of class functions), and zero",
    true,
    [ IsDivisionRing, IsHomogeneousList, IsClassFunction ], 30,
#T increase the rank, for preferring to Gaussian space construction
...
InstallOtherMethod( LeftModuleByGenerators,
    ...
    true,
    [ IsDivisionRing, IsList and IsEmpty, IsClassFunction ], 30,
#T increase the rank, for preferring to Gaussian space construction
#T needed here??
    ...
    SetZero( V, zero );
    V!.elementsunderlying:= UnderlyingCharacterTable( zero );
    return V;
    end );
...
InstallOtherMethod( FLMLORByGenerators,
    "for division ring, homog. list (of class functions), and zero",
    true,
    [ IsDivisionRing, IsHomogeneousList, IsClassFunction ], 0,
    function( F, gens, zero )
    local tbl, V;
...
InstallMethod( UglyVector,
    ...
    function( V, r )
    return ClassFunction( V!.elementsunderlying, r );
    end );
...
InstallOtherMethod( ScalarProduct,
    ...
    [ IsFreeLeftModule and IsClassFunctionsSpaceRep,
      IsClassFunction, IsClassFunction ], 0,
    function( V, x1, x2 )
    return ScalarProduct( V!.elementsunderlying, x1, x2 );
    end );
