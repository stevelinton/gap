...
local  classes,    	# classes to be constructed, the result
  ...
  if r>0 then
    ...
    decomp:=Immutable(decomp);
    ConvertToMatrixRep(decomp,f);
...
# assumes G simple!
  ...
  repeat
    ...
    Gi:=ImagesSet(ai,Gi);
    iso:=iso*ai;
    ai:=List(autos,i->GroupHomomorphismByImages(Gi,Gi,GeneratorsOfGroup(Gi),
    ...
    if ForAll(ai,IsConjugatorAutomorphism) then
      ...
      ai :=List(ai,ConjugatorInnerAutomorphism);
    ...
    repeat
      ...
      # arbitrary value.
    until Index(G,u)>1 and Index(G,u)<cnt*Length(dom);
    # next attempt at iso
  ...
  for k in clop do
	...
	while Length(clTR)>0 do
	  ...
	  while Size(centralizers[j])>(Sum(orb,i->Size(i[3]))*Size(stab)) do
		...
		if ForAll(GeneratorsOfGroup(centralizers_r[j]),
		          i->i in ug[genpos])
		 then
		...
		if p=fail then
		  ...
		  if Length(select)>0 then
		    ...
		    if diff<0 then
		      # only go through this if the obit actually grew
		      # larger
		      minlen:=Sum(orb,i->Size(i[3]));
		      repeat
			while not IsInt(Size(centralizers[j])/minlen) do
			  minlen:=minlen+gcd;
			od;
...
local cs,	# chief series of G
  ...
  if HasAbelianFactorGroup(G,cs[2]) then
    ...
    hom:=NaturalHomomorphismByNormalSubgroup(G,cs[2]);
  ...
  for i in [3..Length(cs)] do
	...
	Thom:=NaturalHomomorphismByNormalSubgroup(M,subN);
	T1:=Image(Thom,M);

	autos:=List(GeneratorsOfGroup(G),
		  i->GroupHomomorphismByImages(T1,T1,GeneratorsOfGroup(T1),
	...
	Thom:=NaturalHomomorphismByNormalSubgroup(T[1],N);
	T1:=Image(Thom,T[1]);
	...
	if Index(F,FM)=1 then
	  ...
	  Qhom:=NaturalHomomorphismByNormalSubgroup(F,FM);
	  Q:=Image(Qhom);
	  FQhom:=Fhom*Qhom;
...
local r,	#radical
  ...
  if not ForAll(ser,i->IsNormal(G,i)) then
    ...
    ser:=InvariantElementaryAbelianSeries(r,
             List(GeneratorsOfGroup(G),i->ConjugatorAutomorphism(r,i)));
