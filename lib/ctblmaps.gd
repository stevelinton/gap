...
##  and character constants (see~"IsChar") are not allowed in maps.
##
...
##
##  For a parametrized map <paramap>, `InverseMap' returns a parametrized map
##  whose $i$-th entry is unbound if $i$ is not in the image of <paramap>,
