            ...
            for pelm  in PrimePowerComponents(elm)  do
                ...
                for j  in [ x-1, x-2 .. 0 ]  do
                    ...
                    if not qelm in low  then
                        Add( comp, low );
                        low := ClosureSubgroupNC( low, qelm );
                    fi;
                od;
            od;
        od;
...
InstallMethod( ElementaryAbelianSeriesLargeSteps,
    "remove spurious factors", true, [ IsGroup ], 0,
  DoEASLS);

InstallOtherMethod( ElementaryAbelianSeriesLargeSteps,
  "remove spurious factors", true,[IsList],0,
  DoEASLS);
...
InstallMethod( ClosureGroup, "groups with cheap membership test", IsCollsElms,
  [IsGroup and CanEasilyTestMembership,IsMultiplicativeElementWithInverse],0,
  ClosureGroupIntest);
...
InstallMethod( ClosureGroup, "for group and empty element list", true,
    [ IsGroup, IsList and IsEmpty ], 0,
function( G, nogens )
  return G;
end );
...
##
InstallMethod( ClosureSubgroupNC, "for group with parent, and object",
  true, [ IsGroup and HasParent, IsObject ], 0,
function( G, obj )
...
InstallMethod( ClosureSubgroup, "for group with parent, and object",
  IsCollsElms, [ IsGroup and HasParent, IsMultiplicativeElementWithInverse ], 0,
function( G, elm )
  if not elm in Parent(G) then
    Error("<elm> must be in the parent of <G>");
  fi;
  return ClosureSubgroupNC( G, elm );
end );

InstallMethod( ClosureSubgroup, "for group with parent, and object",
  IsIdenticalObj, [ IsGroup and HasParent, IsObject ], 0,
function( G, obj )
  if not IsSubset(Parent(G),obj) then
    Error("<obj> must be subset of the parent of <G>");
  fi;
...
InstallMethod(IsConjugate,"subgroups",IsFamFamFam,[IsGroup, IsGroup,IsGroup],0,
function(g,x,y)
...
#M  GroupString( <M> )
##
InstallMethod(GroupString, "for a group", true, [ IsGroup,IsString ], 0,
function( G,nam )
local s,b;
  if HasName(G) then
    s:=Name(G);
...
InstallMethod( CanComputeIndex,"if sizes can be computed",IsIdenticalObj,
  [IsGroup and CanComputeSize,IsGroup and CanComputeSize],0,
function(G,U)  
...
InstallMethod( CanComputeIsSubset,"if membership test works",IsIdenticalObj,
  [IsDomain and CanEasilyTestMembership,IsGroup and HasGeneratorsOfGroup],0,
  ReturnTrue);

InstallMethod( CanComputeIsSubset,"default: no, unless identical",
  IsIdenticalObj, [IsDomain,IsDomain],0,
  function(a,b)
