...
##  `WordsTom' and `StandardGeneratorsInfo' (see~"WordsTom").
##  The `WordsTom' value encodes each generator $g$ as a word $w$ in terms of
##  generators of a free group $F$,
##  such that $w$ evaluates to $g$ when the free generators of $F$ are
##  mapped to standard generators of $G$.
##  This means that on the one hand, standard generators of $G$ must be known
##  in order to use `WordsTom'.
##  On the other hand, the words allow one to compute easily generators not
##  only of a subgroup $U$ of $G$ but also generators of the image of $U$
##  in any representation of $G$, provided that one knows standard generators
##  of the image of $G$ under this representation
...
##  $i$.
##
##  There is no default method to compute the `GeneratorsSubgroupsTom' value
##  of a table of marks.
...
##
##  Let <tom> be the table of marks of the group $G$, say,
##  with `IsTableOfMarksWithGens' value `true'.
...
##
##  `StandardGeneratorsInfo' returns a record with the components
##  \beginitems
##  `generators' &
##      a string of names of the standard generators,
##
##  `description' &
##      a string describing how standard generators for the isomorphism type
##      of $G$ are defined, in terms of the `generators' value, and
##
##  `ATLAS' &
...
##
##  There are no default methods to compute the values of `WordsTom' and
##  `StandardGeneratorsInfo' of a table of marks if they are not yet stored.
##  These values are known for all tables of marks that belong to the
##  {\GAP} library of tables of marks (see~"The Library of Tables of Marks").
##
##  If the `StandardGeneratorsInfo' value of a table of marks <tom> is stored
##  then the `GeneratorsOfGroup' value of the underlying group $G$ of <tom>
##  are in fact standard generators for $G$.
##
...
##  `IsTableOfMarks and HasWordsTom' and
...
DeclareOperation( "RepresentativeTomByGenerators",
    [ IsTableOfMarks and HasWordsTom, IsPosInt, IsHomogeneousList ] );
...
##
#9
##  A *straight line program* in {\GAP} is represented by a nonempty list $l$
##  of elements of a free group $F$.
##  The result of such a program when applied to a list <gens> of group
##  elements is defined as follows.
##  Let $n$ be the length of <gens>.
##  In the first step, the image $w_1$ of $l[1]$ under the homomorphism
##  defined by mapping <gens> to the first $n$ generators of $F$ is computed.
##  In the $i$-th step, the image $w_i$ of $l[i]$ under the homomorphism
##  defined by mapping <gens> to the first $n$ generators of $F$ and
##  $w_1, w_2, \ldots, w_{i-1}$ to the next $i-1$ generators of $F$ is
##  computed.
##  If $l$ has length $k$ then $w_k$ is the result of the program.
##


#############################################################################
##
#F  ResultOfStraightLineProgram( <genslist>, <wordslist> )
##
##  `ResultOfStraightLineProgram' evaluates the straight line program given
##  by <wordlist> with generators in the list <genslist>.
##  First evaluate the first word of <wordlist> using <genslist>,
##  usually the generators of `UnderlyingGroup' or equivalent ones.
...
##
DeclareGlobalFunction( "ResultOfStraightLineProgram" );


#############################################################################
##
#F  ConvWordsTom( <wordlist>, <numgens>[, <fam>] )
##
##  `ConvWordsTom' converts the whole list of words from external to
##  internal representation.
##  This is mainly used by tables of marks from the {\GAP} library of tables
##  of marks (see~"The Library of Tables of Marks").
##
...
#O  PossibleFusionsCharTableTom( <tbl>, <tom> ) . . . . . . .  element fusion
##
