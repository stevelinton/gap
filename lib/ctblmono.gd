...
##  again, instead the same group object should be reused, and thus its
##  character table need to be comouted only once.
##  For example,
...
##  the group with order $n$, see~"MinimalNonmonomialGroup".
##
...
##
DeclareAttribute( "TestRelativelySM", IsCharacter );
...
##
DeclareProperty( "IsRelativelySM", IsCharacter );
...
##
##  is a minimal nonmonomial group described by the parameters
