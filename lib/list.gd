...
#T Does this iterator refer to the *current* value of list?
#T (I think it is meaningless to talk about an ``iterator'' if the length
#T of the list may vary and the entries may be exchanged ...)
#T What about the mutability of the elements in <list>?
#T Currently `ShallowCopy' is called in order to make the top level of the
#T argument independent of the list used by the iterator;
#T but the elements of <list> may be mutable, and it may be necessary that
#T the iterator returns mutable objects;
#T one may even want to guarantee that exactly the objects stored in <list>
#T are returned by the iterator.
#T So what is the exact definition of `IteratorList'?
##
