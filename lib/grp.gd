...
#A  PClassPGroup( <G> )
##
##  The $p$-class of a $p$-group <G> is the length of the lower $p$-central
##  series (see~"PCentralSeries") of <G> for a prime $p$.
...
#A  RankPGroup( <G> )
##
##  The rank of a $p$-group <G> is the minimal size of a generating system
##  of <G>.
...
##  that omit exactly one prime. Every pair of subgroups from this set commutes as subgroups.
...
##
##  These three functions implement the three main methods for closure
##  group. In the order given, they just add <elm> to the generators, remove
...
#O  ClosureSubgroupNC( <G>, <obj> )
##
##  As `ClosureGroup' but makes the result a subgroup of the parent of <G>
##  again. If <obj> is not in this parent, an error is raised; this test is
##  omitted by the `NC' version.
DeclareOperation( "ClosureSubgroup", [ IsGroup and HasParent, IsObject ] );
DeclareOperation( "ClosureSubgroupNC", [ IsGroup and HasParent, IsObject ] );
