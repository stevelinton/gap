...
local   pa, l, map,  d,lc,ro;
  ...
  while d<=l do
    if not IsBound(map[d])  then
	return elm;
    elif d>=pcgs!.tailStart then
      # from this level on every level in the parent is also in the pcgs,
      # so we can clean out completely
      return OneOfPcgs(pcgs);
    fi;
    # was: elm := ReducedPcElement( pa, elm, pcgs[map[d]] );
    #elm := LeftQuotient(pcgs[map[d]]^
    #                       (LeadingExponentOfPcElement(pa,elm)/lc[d]
    #		    mod ro[d]),elm);
