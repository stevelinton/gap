...
##
##  tests whether the objects <obj1> and <objs2> are identical (that is they
##  are either immediate objects or both point to the same bag).
##
...
##
##  A {\GAP} object is *copyable* if one can construct a mutable version of
##  it via `ShallowCopy'.
##  For example, lists, records, and iterators are copyable,
##  integers and domains are not copyable.
##
...
##
##  No other filter must imply `IsMutable',
...
#O  String( <obj>, <length> ) . . . . . .  string representation of an object
##
...
##  
##  In the two argument case, the string returned should be mutable
##  and "fresh" (ie not a part of any other object) so that it can
##  be modified safely, or MakeImmutable may be safely applied to it.
##
...
##
##  is the function used by `String' when this is called with two arguments.
##
DeclareOperation( "FormattedString", [ IsObject, IS_INT ] );
...
##
##  Displays the object <obj> in a nice, formatted way. The actual format
