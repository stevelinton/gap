...
local ac,bc,i,j,pc,pv,brci;
  ...
  for i in [1..Length(ac)] do
    ...
    od;
  od;
  return LaurentPolynomialByCoefficients(brci[1],pc,pv,brci[2]);
end);
...
BindGlobal("RPGcdCRT",function(f,p,g,q,brci)
local min, cf, lf, cg, lg, i, P, m, r;
  ...
  return LaurentPolynomialByCoefficients(brci[1],cf,min,brci[2]);
...
local G, P, l, m, i;
  ...
  od;
  G := LaurentPolynomialByCoefficients(t.brci[1],l,G[2],t.brci[2]);
  Info(InfoPoly,3,"gcd mod ",t.modulo," = ",G);
...
local brci,gcd,fam,fc,gc;
  ...
  return StandardAssociate(R,LaurentPolynomialByExtRep(fam,[gcd,fc],brci[2]));
end);
...
local   m, n, i, k, c, q, val, fc,gc,brci;
  ...
  od;
  return LaurentPolynomialByCoefficients(brci[1],q,val,brci[2]);
...
##
InstallOtherMethod(FactorsSquarefree,"univariate rational poly",true,
