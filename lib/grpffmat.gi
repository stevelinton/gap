...
InstallGlobalFunction( NicomorphismOfFFEMatrixGroup, function( grp )
    ...
    if (HasIsNaturalGL( grp ) and IsNaturalGL( grp ))
       ...
       or IsFullSubgroupGLorSLRespectingBilinearForm( grp )
       or IsFullSubgroupGLorSLRespectingSesquilinearForm( grp )  then
...
function( G, flag )
  local   mycartesian,  fill,  myval, pols,  nrpols,  pairs,  
          types,  a,  res,  a2,  pos,  i,  tup,  arr,  mat,  cen,  b,  
...
InstallMethod( ConjugacyClasses,
               "for natural gl",
               true,
               [IsGroup and IsNaturalGL],
               SUM_FLAGS,
function( G )
    return ConjugacyClassesOfNaturalGroup( G, false );
end );
...
InstallMethod( ConjugacyClasses,
               "for natural gl",
               true,
               [IsGroup and IsNaturalSL],
               SUM_FLAGS,
function( G )
    return ConjugacyClassesOfNaturalGroup( G, true );
end );
