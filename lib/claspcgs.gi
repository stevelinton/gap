...
local   zero,  one,  r,  ran,  n,  nan,  cg,  pos,  Q,  i,  j,  v;
    ...
    if IsEmpty( cg.needed )  then
        ...
        cg.inverse:=Q{ Length( gens ) + ran }
                       { [ 1 .. Length( cg.needed ) ] };
	MakeImmutable(cg.inverse);
	ConvertToMatrixRep(cg.inverse,p);
    fi;
	...
	MakeImmutable(cg.projection);
	ConvertToMatrixRep(cg.projection,p);
...
local  classes,    # classes to be constructed, the result
	...
	xset:=ExternalSet(SubgroupByPcgs(H,Cgens),aff,gens,imgs,OnPoints);
      ...
      k:=[];
      while next<>fail do
        S:=Pcs_OrbitStabilizer(gens,aff[next],imgs,OnRight);
	# tick off
	for i in S.orbit do
